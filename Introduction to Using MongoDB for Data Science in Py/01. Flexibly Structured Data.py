...
Hint
You can access a database instance by name as an attribute of a connected client instance. Remember to use the .list_*_names() methods -- there are special bookkeeping databases and collections that MongoDB creates on its own.
...
Hint
You can call keys() on a dictionary to get a new view of its keys. You will need to pass this view to Python's list constructor, list, to obtain a list.
...
Hint
You can express that two criteria {"field1": "value1"} and {"field2": "value2"} need to be satisfied by including both criteria in the same filter document.
...
Hint
You can express that multiple criteria {"field1": "value1"}, {"field2": "value2"}, etc. need to be satisfied by include all criteria in the same filter document.
...
Hint
The "country" field is in a subdocument in an "affiliations" array, where "affiliations" in turn is a field of a subdocument in a root-level "prizes" array. To reach into this substructure, we use MongoDB dot notation.
...
Hint
Use dot notation and the not-equal ($ne) operator to specify a query constraint for the prize affiliation country, prizes.affiliations.country. Also, recall that the "bornCountry" field stores a laureate's country of birth.
