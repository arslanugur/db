...
We can sort by multiple columns. On the left,the columns are first sorted by year_intro in descending order, and then channels in ascending order. Year_intro is listed first in the ORDER BY clause, so it takes precedence when sorting the rows. Then the channels column is sorted in ascending order, for each year. On the right, both columns are sorted in descending order.
...
We can filter based on numeric values, and also strings or dates. We use single quotes to specify the text or date value we want to use as our filtering criteria. As with ORDER BY, we can use a column that is not in the main SELECT statement, as part of our WHERE clause.
...
Some columns are not allowed to have missing values. However, it may be perfectly valid for other columns to contain NULLs. A missing value is not necessarily a zero value, and we may need to be aware of the 'missingness' of the record - NULLS help highlight gaps in our data.
...
1. Aggregating Data
To start to get an understanding of a new data set, it's common to count the number of observations and summarise any numeric columns. In this lesson, we will learn how to perform this using T-SQL. The examples use the grid database.

2. SUM - single column
It's straightforward to obtain the sum of a single column. Starting with SELECT, type SUM, then open parentheses, then type the column name, then close the parentheses. You should also provide a meaningful alias, otherwise the results will be returned as (No column name), which will get confusing very quickly
...
5. Use aliases
So what's wrong here? That's right, I forgot to provide an ALIAS for my column names, and so I get these less than helpful defaults. You probably aren't going to remember what these columns are, so don't forget to use AS to provide a meaningful name for your results.
...
2. A simple SELECT
Here is a simple query, returning one value, representing the total SUM of the demand_loss column from the grid table. We'll almost always need to dig deeper below a headline figure. If we wanted a breakdown by the individual power outage descriptions - how could we achieve this?

3. Grouping error
We might try adapting the previous query by adding the description field to the SELECT statement. But no, we get an error message. The error is telling us that the description field is invalid. It either needs to have an aggregate function applied to it, or it needs to appear in a GROUP BY clause.
...
Here's a slightly modified version of the query. We've filtered out NULL values from the demand_loss column, and restricted the description to fields that end with 'storm'. This gives us 7 separate groups, with total lost demand for each.
...
We have a table of patient admissions and another showing patient discharges. The admissions table has 5 rows, and the discharges table has 3. An INNER JOIN on those 2 tables shows the patients who had been admitted AND discharged - patients 1,3 and 4. But that doesn't tell us about current patients. In order to get the complete picture in one result set, we need a LEFT JOIN. We need all the rows from the admissions table, so that becomes the table on the left-hand side of the join. We also want to return all matches and identify non-matches from the discharges table. Any non-matches will return a NULL value in the discharge column. Now we have a complete result set showing patients who have been both admitted and discharged, and those who have been admitted but not yet discharged.
...
9. Summary
Let's recap what we covered: The difference between INNER joins and LEFT or RIGHT joins is that INNER JOINS only ever return matching rows from both tables. LEFT or RIGHT joins return ALL the rows from the main query table, plus any matches from the joining table. If a row in the main table doesn't have a match in the joining table, then a NULL value will appear in any columns you select from the joining table. LEFT and RIGHT joins can be interchangeable - we can re-write a LEFT to a RIGHT and a RIGHT to a LEFT. LEFT joins are more common, but we should be aware of RIGHT joins as they are often useful when multiple tables need to be joined together.
...
2. Two queries
Here's a query which returns a small result set from the album table. We're selecting all three columns, but only where the artist_id is 1 or 3, and the query returns three rows. Here we have a similar query, returning the same three columns, from a similar table, but only where the artist_id is 1, 4 or 5. This returns 4 rows. Notice that this query, and the query before it, both return rows for the artist_id of 1. Our first query returned 3 rows, and the second query returned 4 rows. While the example is querying the same table, in real life we may have data in separate tables, which share the same structure, that we need to combine into one result set. What do we mean by the same structure? It means we expect that both tables have the same number of columns, the columns are listed in the same order and have similar data types. How could we combine their results?

3. Combining results
The answer is to use UNION. We write the first query, then UNION on a new line, then we write the second query below it. How many rows will this query return? The query only returns 5 rows, not the 7 we might have expected. Why? Well, UNION excludes duplicate rows, and as both queries contained the data for artist_id 1, we only retrieve 2 , and not 4 rows for that artist_id.

4. UNION ALL
Here's a slightly different version of the UNION query we saw previously. Can you spot the difference? Instead of UNION to join the two queries, it uses UNION ALL. How many rows might be returned now? This time, we return all 7 rows. UNION ALL returns all the rows from both queries, including duplicates.

5. Creating new column names for final results
In this final example, we show the same UNION ALL query as before, but we provide a new alias for each column. If we have 2 different tables, with different column names, but the same datatypes in the same order, then we can still combine the results , while providing new common column names for our final result set.

6. Summary
Let's recap. We can combine queries from the same table, or from different tables, using UNION or UNION ALL. If we're combining data from different tables, our queries need to SELECT the same number of columns, in the same order, and have the same data types, so that the results can be combined. If our source tables have different column names, we can ALIAS the column names in our SELECT queries so that the final results share the same column names. UNION combines results but discards duplicates. This means it can be slower to run, whereas UNION ALL returns all results including duplicates, so it is faster.
...
In this final chapter, we'll add to our knowledge by learning to create and update tables, and also how to insert and delete rows. These are commonly described as CRUD operations, and we'll briefly cover what we mean by that acronym. Then, we'll mainly focus on creating tables. In order to do that, we need to think about the type of data we want to store in each column.

2. CRUD operations
The acronym CRUD describes the 4 main type of operations you can carry out on a database. First, we can CREATE - in a data sense, that could be tables or views, but in addition, database administrators can also create users, permissions and security groups. Next, we perform READ operations. When we perform a SELECT query, we are reading from the database. We can amend existing database records using UPDATE statements. Finally, we can DELETE records, although this depends on having sufficient access permissions to carry this out. Let's look at CREATE in more detail.

3. CREATE
We can create a table by using the keywords CREATE TABLE and then proving a table name. Each table name within a database should be unique. Then, we open parenthesis and type the column name, a comma, then the data type for that column, and if appropriate, the size, length or format of that data type in brackets. Each column is defined on a separate line, before closing the parenthesis. For example, we can create a table named `test_table`. This table has 3 columns, test_date, which is a date column, test_name which has the datatype varchar, and test_int which is an integer column. Notice that test_name has the varchar datatype with the value 20 in brackets immediately after. This value refers to the maximum number of characters that can be stored in this column. So in this case, we can store anything up to 20 characters.

4. A few considerations when creating a table
Creating a table is straightforward, but we should spend some time planning it beforehand. As well as table and column names, we'll need to think about the type of data each column will store, and where appropriate, the size or amount of data stored in the column.

5. Data types
Here are some of the more common data types used in SQL Server. We can store dates, times or combined datetime values. The default date format is YYYY-MM-DD. Numeric values can be stored as integers, decimals, or bit, which stores either a 1 or 0, relating to TRUE or FALSE. For strings, common types include char, varchar, and nvarchar. I encourage you to research these and the other data types available, so you are aware of which best suits your needs.
...
1. Declare yourself
In this lesson we'll look at the use of variables - why they are useful, how to create them and assign values to them. We'll also look at creating temporary tables, which can help us carry out analysis tasks when we don't have access or permissions to other tables.

2. Variables
First of all, what do we mean by a variable? It's helpful to think of it as a placeholder for a specific value, of a specific data type. Why is that useful? Well, it means we write less repetitive code. Let's imagine our boss wants all the information from the artist table about AC/DC. That could happen, right? So we write a query and give them the results. Now they want to know about U2. So we write another query. Then they want to know about another artist. And another. Do we really want to write a new query each time? A better way is to create a variable. Variable names begin with the 'at' sign , so we could perhaps call this variable "@my_artist". Then, in the query, rather than writing the artist name, we refer to "@my_artist". And then all we need to do is update the variable each time we run the query. One query, easily modified, for repetitive use. Let's see how to do it.

3. DECLARE
We use the DECLARE keyword to create a variable. Then we type the variable name, beginning with the 'at' symbol. Here are a couple of examples. First, we can define a variable, @test_int, which is an integer. Or, following on from the previous slide, we can create @my_artist, which has the varchar datatype, accepting anything up to 100 characters in length. So that creates a variable, but how do we pass a value to it?

4. SET
In order to assign a value to a variable, we use the keyword 'SET', immediately after the DECLARE statement. Here we see our previous integer variable, @test_int, which we have declared, and now SET to a value of 5 by typing SET, then the variable name, the equals sign, then the chosen value, in this case, 5. Similarly, we can assign the value 'AC/DC' to our @artist variable as shown. Then we can refer to these variables in a WHERE clause.

5. Declaring more than one variable
Here's an example of declaring two variables, in this case and @my_artist, and @my_album. We DECLARE them both first, then SET the values, and then finally we write our SELECT query. We can see that in the WHERE clause, the artist should equal @my_artist, and the album should equal @my_album. If we then need to change the artist and album combination, we simply update the SET section of the query, and run it again.

6. Temporary tables
Imagine we've written a query, joining information from different tables, with nicely aliased column names. It would be great if this was a table that we could use for further work. The good news is, we can create a temporary table. And once we've created it, it's available for further querying for the remainder of our session. How? We write our SELECT query, but in between SELECT and FROM, we use the keyword INTO, and then type our desired table name, prefaced by the hash (#) sign. The table will exist either until our connection or session ends, or we manually remove it with the 'DROP TABLE' statement.
